...
let Saved: (string | null)[] = [null, null, null, null, null, null, null, null, null, null];
let Ans: boolean[] = [false, false, false, false, false, false, false, false, false, false];

let currentIndex: number = 1;
const PrevBtn: HTMLElement = (document.querySelector(".body__wrapper__controlsContainer-prev") as unknown) as HTMLElement
const NextBtn: HTMLElement = (document.querySelector(".body__wrapper__controlsContainer-next") as unknown) as HTMLElement

const Start = (e: HTMLElement) => {
    currentIndex = 1;
    const res:HTMLElement = (document.querySelector(".body__resultscr") as unknown) as HTMLElement;
    res != null ? res.remove() : null;
    const wrapper: HTMLElement = (document.querySelector(".body__wrapper") as unknown) as HTMLElement;
...
const CreateLayout = () => {
    toggleCtrl();
    const ShownIndex: HTMLElement = (document.querySelector(".body__wrapper__indexContainer") as unknown) as HTMLElement;
...
const HandleControls = async (e: HTMLElement, type: boolean) => {
    if (e.classList.contains("inactive")) return;
    if (type) {
        switch (currentIndex) {
            case 1:
...
const Q: HTMLElement = (document.querySelector(".body__wrapper__activeArea-question") as unknown) as HTMLElement;
const A: HTMLElement = (document.querySelector(".body__wrapper__activeArea-answer") as unknown) as HTMLElement;
const ShowQuestion1 = () => {
    ...
    const _input: HTMLTextAreaElement = (document.querySelector(".input") as unknown) as HTMLTextAreaElement;
    if (Saved[0] != null) {
...
const HandleQuestion1 = async () => {
    const data: HTMLTextAreaElement = (document.querySelector(".input") as unknown) as HTMLTextAreaElement;
    Saved[0] = data.value;
...
const HandleQuestion3 = () => {
    const res: HTMLInputElement = (document.querySelector(".input") as unknown) as HTMLInputElement
    Saved[2] = res.value;
...
const HandleQuestion4 = async () => {
    ...
    await fetch(API_URL, {
        ...
        Ans[3] = false;
        if (data["output"] != undefined && (data["output"] as string).toUpperCase().replaceAll("\n", "") == 'LXXXIV') {
            Ans[3] = true
        }
    })
    clearTimeout(timeOut);
...
const ShowQuestion5 = () => {
    ...
    const _inputs: HTMLInputElement[] = (document.querySelectorAll(".input") as unknown) as HTMLInputElement[];
    if (Saved[4] != null) {
...
const HandleQuestion5 = () => {
    Saved[4] = '';
    const answers: HTMLElement[] = (document.querySelectorAll(".active") as unknown) as HTMLElement[];
...
const ShowQuestion6 = () => {
    ...
    if (Saved[5] != null) {
        (((document.getElementById(Saved[5])) as unknown) as HTMLElement).classList.add("active");
    }
}
const HandleQuestion6 = () => {
    const answer: HTMLElement = (document.querySelector(".active") as unknown) as HTMLElement;
...
const ShowResults = () => {
    let score = 0;
    let topScore:number = +(localStorage.getItem("score") as string);
    for (let i=0; i<10; i++){
        if (Ans[i]){
            score++;
        }
    ...
    if (topScore < score){
        topScore = score;
        localStorage.setItem("score", (topScore as unknown) as string);
    }

    const body: HTMLElement = (document.querySelector(".body") as unknown) as HTMLElement;
    ...
    resScr.appendChild(best); resScr.appendChild(curr);
    resScr.appendChild(btn);
    body.appendChild(resScr);
}

const textareaHandler = (e: KeyboardEvent, elem: HTMLTextAreaElement) => {
    if (e.key == "Tab") {
        e.preventDefault();
...
const PrecodeTextArea = (elem: HTMLTextAreaElement) => {
    elem.value = `using System;\npublic class Program\n{\n\tpublic static void Main()\n\t{\n\t\t\n\t}\n}`
}

const rbHandler = (activeElem: HTMLElement) => {
...
const cbHandler = (caller: HTMLInputElement) => {
